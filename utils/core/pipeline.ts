...
import { buildEmbeddingText } from "../lib/utils";
...
export async function runPipeline(
  ...
  reportProgress(
    onProgress,
    "recalling",
    `共召回 ${candidates.length} 个候选项目`,
  );

  if (candidates.length === 0) {
    return { repo: repoData, features, results: [] };
  }

  // Step 4a: 快速过滤 — 选出 top 30
  reportProgress(onProgress, "filtering", "正在快速过滤候选...");
  const fastCandidates = rankFast(
    candidates,
    repoData.topics,
    repoData.description || "",
  );
  reportProgress(
    onProgress,
    "filtering",
    `快速过滤完成，保留 ${fastCandidates.length} 个`,
  );
  ...
  reportProgress(
    onProgress,
    "fetching-readmes",
    `正在并发获取 README... (0/${fastCandidates.length})`,
  );
  await fetchReadmesConcurrent(fastCandidates, config.githubToken);
  reportProgress(
    onProgress,
    "fetching-readmes",
    `README 获取完成 (${fastCandidates.length} 个)`,
  );
